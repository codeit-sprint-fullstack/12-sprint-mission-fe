...
![demo](/src/assets/images/readme-img/demo.gif)
...
![main](/src/assets/images/readme-img/main-preview.png)

### 로그인 페이지

![login](/src/assets/images/readme-img/login-preview.png)

### 회원가입 페이지

![signup](/src/assets/images/readme-img/signup-preview.png)

### 상품 목록 페이지

![product](/src/assets/images/readme-img/product-preview.pngsrc)
...
![api-test](/src/assets/images/readme-img/api-test.PNG)
...
판다마켓 서비스의 **상품 목록 페이지를 직접 구현**하고,
이후 **기능 개선 및 구조 리팩토링을 통해 완성도를 높이는 것**을 목표로 했습니다.
...
- **커스텀 훅 구조 설계 (useProducts)**
  - 상품 목록 / 베스트 상품 상태 통합 관리
  - 로딩 / 에러 처리
  - 페이지 / 검색 / 정렬 상태 관리

- **반응형 처리**
  - 화면 크기에 따라 pageSize 자동 변경
    - Desktop: 10
    - Tablet: 6
    - Mobile: 4

- **UI/UX 개선**
  - 이미지 비율 통일 (aspect-ratio + object-fit)
  - 카드 레이아웃 정리
  - 페이지네이션 버튼 원형 UI 적용
...
- API 기반 리스트 페이지 설계 및 구현 방법
- 커스텀 훅을 활용한 상태 관리 구조화
- 페이지네이션 로직 직접 구현 경험
- debounce를 활용한 입력 최적화
- 반응형 UI 설계
...
- HTML 구조 설계 및 UI 레이아웃 구현
- CSS 변수 활용 및 반응형 디자인
- REST API CRUD 이해
- 비동기 처리 방식 학습 (then / catch, async / await)
- LocalStorage 활용 장바구니 기능 구현
- 페이지네이션 재사용 컴포넌트 구현
...
### 4️⃣ 4차 미션 - 페이지네이션 계산 오류

#### 문제

상품 목록 페이지에서 페이지네이션을 구현하는 과정에서
페이지 이동 시 데이터가 정상적으로 표시되지 않는 문제가 발생했습니다.

즉

- 특정 페이지에서 상품이 비어 보임
- 페이지 수가 실제 데이터보다 적거나 많게 표시됨

#### 해결

전체 상품 수(`totalCount`)를 기준으로
전체 페이지 수를 계산하도록 수정했습니다.
...
페이지 수가 실제 데이터와 일치하도록 개선되었고
페이지 이동 시 안정적으로 상품 목록이 출력되도록 수정되었습니다.

---

### 4️⃣ 4차 미션 — 상품 목록 데이터 미출력 문제

#### 문제

페이지(`page`) 또는 페이지 크기(`pageSize`)가 변경될 때
상품 목록이 제대로 갱신되지 않는 문제가 발생했습니다.

즉

- 페이지 이동 시 이전 데이터가 유지됨
- 일부 경우 데이터가 아예 표시되지 않음

#### 해결

`useEffect`의 의존성 배열에
`page`, `pageSize`, `orderBy`, `keyword`를 추가하여
상태 변경 시마다 데이터를 다시 요청하도록 수정했습니다.
...
페이지 이동 및 조건 변경 시마다
최신 데이터가 정상적으로 반영되도록 개선되었습니다.

---

### 4️⃣ 4차 미션 — 페이지네이션 버튼 UI 깨짐

#### 문제

페이지네이션 버튼이 의도한 것과 다르게
원형이 아닌 타원 형태로 표시되는 문제가 발생했습니다.
...
버튼의 `width`, `height`를 동일하게 설정하고
`border-radius: 50%`를 적용하여 완전한 원형으로 수정했습니다.
...
페이지네이션 버튼이 일관된 원형 UI로 표시되었고
전체 레이아웃의 완성도가 개선되었습니다.
