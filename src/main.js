...
const runProductTests = async () => {
  ...
  if (createdProduct && createdProduct.id) {
    ...
    console.log(
      `✅ [Product] ${createdProduct.id}번 상세 조회:\n`,
      productDetail,
    );
  }
  ...
  if (createdProduct && createdProduct.id) {
    ...
    console.log(
      `✅ [Product] ${createdProduct.id}번 수정 결과:\n`,
      updatedProduct,
    );
  }
  ...
  if (createdProduct && createdProduct.id) {
    ...
    console.log(
      `✅ [Product] ${createdProduct.id}번 삭제 완료:\n`,
      deletedProduct,
    );
  }
};
...
ArticleService.createArticle(newArticleData)
  .then((createdData) => {
    console.log("✅ 생성:", createdData);
    createdId = createdData.id;
    return ArticleService.getArticle(createdId);
  })
  .then((detailData) => {
    ...
    return ArticleService.patchArticle(createdId, updateData);
  })
  .then((updatedData) => {
    console.log("✅ 수정:", updatedData);
    return ArticleService.deleteArticle(createdId);
  })
  .then((deleteData) => {
